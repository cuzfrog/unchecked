...
import static com.github.cuzfrog.function.Functionally.*;
...
public final class FunctionallyTest {
    ...
    public void uncheckedPredicate() {
        needPredicate(uncheckedP(this::test));
    }
    ...
    private <T> boolean test(T t) throws IOException {
        throw new IOException("Man made exception.");
    }

    private static <T> void needPredicate(Predicate<T> predicate) {
        predicate.test(null);
    }
}
